...
if __name__ == '__main__':
    ...
    for i in tqdm(range(test_round)):
        cws_results = cws_model.predict(inputs, device=device)
        if i == 0:
            for result in cws_results:
                print(result)
            print('#' * 100)
            print('\n')
        pos_results = pos_model.predict(cws_results, device=device)
        if i == 0:
            for result in pos_results:
                print(result)
            print('#' * 100)
            print('\n')
        ner_results = ner_model.predict(inputs, device=device)
        if i == 0:
        ...
        dep_results = dep_model.predict(tok_pos_inputs, task='dep', device=device)
        if i == 0:
        ...
        for tokens, poses in zip(cws_results, pos_results):
            ...
            for tok, pos in zip(tokens, poses):
                tok_pos_input.append((tok, pos))
            tok_pos_inputs.append(tok_pos_input)
        sdp_results = sdp_model.predict(tok_pos_inputs, task='sdp', device=device)
        if i == 0:
